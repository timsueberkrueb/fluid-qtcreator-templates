...
class QmakePlugin(snapcraft.BasePlugin):
    ...
    @staticmethod
    def _build_qt_path(*fragments, qt_path=None):
        if not qt_path:
            qt_path = os.environ['QTDIR']
        return os.path.join(qt_path, *fragments)
    ...
    def _extra_config(self):
        ...
        for root in [self.installdir, self.project.stage_dir]:
            ...
            extra_config.append("LIBS+=\"-L{}\"".format(QmakePlugin._build_qt_path('lib')))
            extra_config.append("QMAKE_LIBS+=\"-L{}\"".format(QmakePlugin._build_qt_path('lib')))
            extra_config.append("QMAKE_LIBDIR+=\"{}\"".format(QmakePlugin._build_qt_path('lib')))

            paths = common.get_include_paths(root, self.project.arch_triplet)
            for path in paths:
                extra_config.append("INCLUDEPATH+=\"{}\"".format(path))
            extra_config.append("INCLUDEPATH+=\"{}\"".format(QmakePlugin._build_qt_path('include')))

            extra_config.append("QML_IMPORT_PATH+=\"{}\"".format(QmakePlugin._build_qt_path('qml')))
            extra_config.append("QML2_IMPORT_PATH+=\"{}\"".format(QmakePlugin._build_qt_path('qml')))

        return extra_config

    def _build_environment(self):
        env = os.environ.copy()
        env['QML_IMPORT_PATH'] = QmakePlugin._build_qt_path('qml')
        env['QML2_IMPORT_PATH'] = QmakePlugin._build_qt_path('qml')
        env['LD_LIBRARY_PATH'] = QmakePlugin._build_qt_path('qml')
        env['PATH'] = QmakePlugin._build_qt_path('bin') + ':' + os.environ['PATH']
        return env
